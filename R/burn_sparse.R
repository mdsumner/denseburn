...
materialise_chunk <- function(x, id = NULL, type = c("matrix", "vector")) {
  stopifnot(inherits(x, "denseburn"))
  type <- match.arg(type)
...
print.denseburn <- function(x, ...) {
  ...
  cat(sprintf("<denseburn> %d x %d grid, %d geometr%s\n",
