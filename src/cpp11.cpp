// Generated by cpp11: do not edit by hand
// clang-format off


#include "cpp11/declarations.hpp"
#include <R_ext/Visibility.h>

// denseburn.cpp
cpp11::writable::list cpp_burn_sparse(cpp11::list wkb_list, double xmin, double ymin, double xmax, double ymax, int ncol, int nrow);
extern "C" SEXP _denseburn_cpp_burn_sparse(SEXP wkb_list, SEXP xmin, SEXP ymin, SEXP xmax, SEXP ymax, SEXP ncol, SEXP nrow) {
  BEGIN_CPP11
    return cpp11::as_sexp(cpp_burn_sparse(cpp11::as_cpp<cpp11::decay_t<cpp11::list>>(wkb_list), cpp11::as_cpp<cpp11::decay_t<double>>(xmin), cpp11::as_cpp<cpp11::decay_t<double>>(ymin), cpp11::as_cpp<cpp11::decay_t<double>>(xmax), cpp11::as_cpp<cpp11::decay_t<double>>(ymax), cpp11::as_cpp<cpp11::decay_t<int>>(ncol), cpp11::as_cpp<cpp11::decay_t<int>>(nrow)));
  END_CPP11
}
// denseburn_init.cpp
void cpp_denseburn_init();
extern "C" SEXP _denseburn_cpp_denseburn_init() {
  BEGIN_CPP11
    cpp_denseburn_init();
    return R_NilValue;
  END_CPP11
}
// scanline_burn.cpp
cpp11::writable::list cpp_scanline_burn(cpp11::list wkb_list, double xmin, double ymin, double xmax, double ymax, int ncol, int nrow);
extern "C" SEXP _denseburn_cpp_scanline_burn(SEXP wkb_list, SEXP xmin, SEXP ymin, SEXP xmax, SEXP ymax, SEXP ncol, SEXP nrow) {
  BEGIN_CPP11
    return cpp11::as_sexp(cpp_scanline_burn(cpp11::as_cpp<cpp11::decay_t<cpp11::list>>(wkb_list), cpp11::as_cpp<cpp11::decay_t<double>>(xmin), cpp11::as_cpp<cpp11::decay_t<double>>(ymin), cpp11::as_cpp<cpp11::decay_t<double>>(xmax), cpp11::as_cpp<cpp11::decay_t<double>>(ymax), cpp11::as_cpp<cpp11::decay_t<int>>(ncol), cpp11::as_cpp<cpp11::decay_t<int>>(nrow)));
  END_CPP11
}

extern "C" {
static const R_CallMethodDef CallEntries[] = {
    {"_denseburn_cpp_burn_sparse",    (DL_FUNC) &_denseburn_cpp_burn_sparse,    7},
    {"_denseburn_cpp_denseburn_init", (DL_FUNC) &_denseburn_cpp_denseburn_init, 0},
    {"_denseburn_cpp_scanline_burn",  (DL_FUNC) &_denseburn_cpp_scanline_burn,  7},
    {NULL, NULL, 0}
};
}

extern "C" attribute_visible void R_init_denseburn(DllInfo* dll){
  R_registerRoutines(dll, NULL, CallEntries, NULL, NULL);
  R_useDynamicSymbols(dll, FALSE);
  R_forceSymbols(dll, TRUE);
}
